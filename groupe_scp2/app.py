...
menu = st.sidebar.radio("Aller à :", ["📊 Analyse des données existantes", "🕷️ Lancer un nouveau Scraping"])
...
if menu == "📊 Analyse des données existantes":
    ...
    if df.empty:
        st.warning("⚠️ Aucune donnée trouvée. Allez dans l'onglet Scraping pour récupérer des données.")
    else:
        # Métriques globales
        ...
        # Bar chart
        if 'difference_float' in analyzer.df.columns:
            top_ecarts = analyzer.df.nlargest(10, 'difference_float')
            st.bar_chart(data=top_ecarts, x='name', y='difference_float', color='#ff4b4b')


elif menu == "🕷️ Lancer un nouveau Scraping":
