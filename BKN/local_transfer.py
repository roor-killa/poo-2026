...
def effectuer_transfert(expediteur: Wallet, destinataire: Wallet) -> None:
    ...
    print(f"Montant à transférer: ", end=' ')
    montant = float(input(""))

    transaction = expediteur.send(montant,destinataire)
    ...
    print(f"""
        📊 Nouveaux soldes:
        {expediteur.proprietaire}: {expediteur.solde} BKN
        {destinataire.proprietaire}: {destinataire.solde} BKN""")
...
def main() -> None:
    ...
    # ------------------------------------------------------------------ #
    wallet1 = Wallet('Alice',1000,'LOCAL')  # À remplacer
    wallet2 = Wallet('Bob',500,'LOCAL')  # À remplacer
